...
public class IdPManagementConstants {
    ...
    public static class SQLQueries {
        ...
        public static final String GET_CONNECTED_APPS = "SELECT APP_NAME FROM (SP_AUTH_STEP INNER JOIN " +
                ...
                " ID FROM IDP WHERE NAME = ? AND TENANT_ID = ?)) UNION SELECT APP_NAME FROM SP_PROVISIONING_CONNECTOR" +
                " INNER JOIN SP_APP ON SP_PROVISIONING_CONNECTOR.APP_ID=SP_APP.ID WHERE IDP_NAME = ? AND " +
                "SP_PROVISIONING_CONNECTOR.TENANT_ID = ?";
    }
}
